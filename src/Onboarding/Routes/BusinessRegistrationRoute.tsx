...
export default observer(function BusinessRegistrationRoute() {
  ...
  const canSubmit = !!companyPan && !!gstReg;

  const handleSubmit = async () => {
    setTouched(true);
    if (!canSubmit || isLoading) return;
    const ok = await onboardingStore.submitBusinessRegistration(companyPan!, gstReg!);
    if (ok) router.replace(routeToOnboardingStep(sessionStore.onboardingCurrentStep, sessionStore.onboardingStatus));
  };

  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <StepHeader currentStep={3} totalSteps={9} title="Business Registration" subtitle="Upload your company PAN and GST registration certificate." onBack={() => router.back()} />
