...
export default observer(function IdentityDocsRoute() {
  ...
  const canSubmit = !!panFile && !!aadhaarFile;

  const handleSubmit = async () => {
    setTouched(true);
    if (!canSubmit || isLoading) return;
    const ok = await onboardingStore.submitIdentityDocs(panFile!, aadhaarFile!);
    if (ok) router.replace(routeToOnboardingStep(sessionStore.onboardingCurrentStep, sessionStore.onboardingStatus));
  };

  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <StepHeader currentStep={3} totalSteps={8} title="Identity Documents" subtitle="Upload your PAN card and Aadhaar card." onBack={() => router.back()} />
